...
<template>
  ...
  <p class="muted intro">These apply immediately and are stored in the database — no redeploy needed.</p>
  ...
  <div v-else-if="model" class="settings">
    ...
    <aside class="settings-nav">
      <input v-model="search" class="nav-search" placeholder="Search settings…" />
      <nav>
        <button
          v-for="g in ui"
          :key="g.key"
    ...
    <div class="settings-pane">
      <p v-if="search && shownGroups.length === 0" class="muted empty">No settings match “{{ search }}”.</p>

      <section v-for="group in shownGroups" :key="group.key" class="pane-section">
        <header class="pane-head">
          <h3>{{ group.label }}</h3>
          <p v-if="group.description && !search" class="muted">{{ group.description }}</p>
        </header>
        ...
        <div v-for="f in group.fields" :key="f.path" class="field">
          ...
          <small v-if="f.help" class="muted field-help">{{ f.help }}</small>
        </div>
      </section>
