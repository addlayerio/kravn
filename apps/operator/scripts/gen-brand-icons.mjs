/**
 * Generate packages/contracts/src/brand-icons.ts — brand logos baked from simple-icons.
 *
 ...
 * plain TS map in @kravn/contracts, so BOTH the operator (IntegrationIcon.vue) and the public
 * website (integrations gallery) render logos from one source. simple-icons is a build-time
 * devDependency only — nothing imports it at runtime. Ids with no matching brand logo fall back
 * to a coloured monogram.
 */
...
ts += `// Brand logos baked from simple-icons (path data + hex per catalog/plugin id), shared by the\n`;
ts += `// operator (IntegrationIcon.vue) and the public website (integrations gallery).\n`;
