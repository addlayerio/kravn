<script lang="ts">
/** One selectable row. `groupId` buckets it under an origin server; `id` is what gets stored. */
...
const props = withDefaults(
  defineProps<{
    items: GroupedItem[];
    /** Metadata per groupId (server display name). Missing → groupId shown raw. */
...
// Auto-expand groups that already have a selection the first time items arrive.
watch(
  () => props.items.length,
  (len) => {
  ...
  <div class="gs">
    <div class="gs-toolbar">
      ...
      <span class="gs-actions">
        <button type="button" class="gs-link" :disabled="filtering" @click="expandAll">{{ t('grouped.expandAll') }}</button>
        <button type="button" class="gs-link" :disabled="filtering" @click="collapseAll">{{ t('grouped.collapseAll') }}</button>
      </span>
    </div>
    ...
    <div class="gs-scroll">
      ...
      <div v-for="g in groupViews" :key="g.groupId" class="gs-group">
        <div class="gs-head" role="button" tabindex="0" @click="toggleExpand(g.groupId)" @keydown.enter="toggleExpand(g.groupId)">
          ...
          <Server class="gs-srv-ic" :size="15" />
...
.gs-srv-ic { color: var(--text-muted); flex: 0 0 auto; }
.gs-gname { font-weight: 600; font-size: 0.9rem; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.gs-count {
  margin-left: auto;
  font-size: 0.72rem;
...
.gs-label { font-size: 0.86rem; white-space: nowrap; flex: 0 0 auto; }
